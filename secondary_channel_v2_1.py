...
def monitor_and_wait_manual_activation(
    master,
    target_system,
    target_component,
):
    ...
    log(
        "Press 'h' or 'l' to activate GUIDED hold. "
    ...
    while True:
        ...
        if user_key not in ACTIVATE_KEYS:
            log(
                ...
                "Use 'h' or 'l' for GUIDED hold "
                f"or '{QUIT_KEY}' to quit."
            )
            continue

        if hold_active:
            log("GUIDED hold is already active in this session.")
            continue
...
def main():
    ...
    try:
        ...
        monitor_and_wait_manual_activation(
            master,
            target_system,
            target_component,
        )
