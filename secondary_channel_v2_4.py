...
def validate_emergency_action():
    valid_actions = {"rtl", "hold", "land"}
    action = EMERGENCY_ACTION.lower()

    if action not in valid_actions:
        raise ValueError(
            "Invalid EMERGENCY_ACTION. "
            "Use one of: 'rtl', 'hold', 'land'."
        )
...
def monitor_heartbeat(monitor_master, command_master, target_system, target_component, action):
    ...
    log(f"Configured emergency action: {action.upper()}")
...
def main():
    ...
    try:
        action = validate_emergency_action()
